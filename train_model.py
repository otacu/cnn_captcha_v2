...
class TrainModel(CNN):
    ...
    def train_cnn(self):
        ...
        with tf.compat.v1.name_scope('cost'):
            cost = tf.reduce_mean(input_tensor=tf.nn.sigmoid_cross_entropy_with_logits(logits=y_predict, labels=self.Y))
        # 梯度下降
        ...
        max_idx_l = tf.argmax(input=tf.reshape(self.Y, [-1, self.max_captcha, self.char_set_len]), axis=2)  # 标签
        # 计算准确率
        correct_pred = tf.equal(max_idx_p, max_idx_l)
        with tf.compat.v1.name_scope('char_acc'):
            accuracy_char_count = tf.reduce_mean(input_tensor=tf.cast(correct_pred, tf.float32))
        with tf.compat.v1.name_scope('image_acc'):
            accuracy_image_count = tf.reduce_mean(input_tensor=tf.reduce_min(input_tensor=tf.cast(correct_pred, tf.float32), axis=1))
        # 模型保存对象
